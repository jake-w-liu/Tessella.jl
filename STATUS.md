...
- **Suite:** `julia --project=. -e 'using Pkg; Pkg.test()'` green — **143,096
  assertions** under `--check-bounds=yes` (verified this session; +28 `mesh_box`,
  +24 `mesh_box_regions`, +36 `recover_boundary` (`steiner` fan fallback; non-star
  prism breadth; exotic twisted-prism blocker), +9 `mesh_sized_conforming` (curved
  interior size control), +32 `mesh_boolean` native mesh-CSG, +11 end-to-end
