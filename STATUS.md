...
- **Suite:** `julia --project=. -e 'using Pkg; Pkg.test()'` green — **143,004
  assertions** under `--check-bounds=yes` (verified this session; +28 from the
  `mesh_box` size-controlled mesher, +24 from `mesh_box_regions` conforming
  multi-region / native box-CSG mesher).
