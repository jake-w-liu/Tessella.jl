...
- **2026-08-13 — "complete ALL" pass: #11 fixed, all 22 HFSS geometries meshed
  ...
  - **Arbitrary-surface *uniform* sizing: assessed, not shipped.** A POC (exact-midpoint
    ...
    **correct in principle** but **impractically slow** — the exact kernel's
    `Rational{BigInt}` cost on the refined vertex count (the same exact-predicate cost
    that is #11's residual), and the Float64 kernel cannot size conformingly (perturbation
    moves boundary points off-feature). Genuinely gated on exact-kernel performance;
    `mesh_sized_cdt` continues to provide interior sizing bounded by the input surface's
    facet edges. *Acceptance test:* on a non-box surface, a sweep of `hmax` yields
    `maxedge ≤ hmax` on BOTH surface and interior with `validate.ok`, exact volume, and
    bounded runtime.
