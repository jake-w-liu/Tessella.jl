...
**Conclusion (measured, not assumed):** a robust 3-D size-controlled mesher needs a substantial new subsystem — either **boundary-conforming Delaunay refinement** (Shewchuk's terminator: interior Steiner points *plus* encroachment-driven boundary-subface splits; divergence-prone near small input angles) or an **explicit-connectivity structured mesher** (emit BCC/lattice tets directly rather than Delaunay-ing degenerate points). Both are research-grade, out of single-session scope. *Test that would confirm a correct implementation:* on the box, over a sweep of `hmax`, `maxedge ≤ hmax` with `validate.ok`, `is_delaunay3` (or explicit validity), exact preserved volume, good min dihedral, and bounded vertex count (termination).
