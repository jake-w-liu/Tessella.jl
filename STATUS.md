...
**Native CSG is shipped** (`mesh_box_regions` + `mesh_boolean`, no OCC). Per the
user's **2026-08-12 descope**, the ASCENT drop-in and 22-case HFSS regression are
**out of scope** (external artifacts absent from this environment). The two genuinely
research-grade meshing items are being **implemented** (not deferred):

| item | status |
|---|---|
| ASCENT drop-in + 22-case HFSS regression | **OUT OF SCOPE (user-descoped 2026-08-12)** — needs the ASCENT binary + 22 proprietary HFSS datasets, external to this env; the implementations they'd test (P2 curving, solver-consumable MSH v4.1) are done |
...
| uniform size control on curved domains | **cylinder DONE** (`mesh_cylinder`, uniform, cospherical-robust); graded for generic curved (`mesh_sized_conforming`); *arbitrary-surface uniform* = general Shewchuk terminator — **in progress** |
| recovery for non-star+reflex polyhedra | conforming-Delaunay boundary-Steiner recovery — **in progress** (workflow); currently raises a safe explicit blocker |
