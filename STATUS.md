...
| 4 | size fields + optimization | **partial** | tet quality report ✓; Laplacian + ODT smoothing ✓; verified 2-3/3-2 flip primitives ✓; `optimize_flips!` sliver reduction (309→169 flips alone, →63 with smoothing on a 300-pt cloud; volume/validity/min-dihedral-safe) ✓; weighted sliver exudation + domain-bounded 3-D refinement WIP — *measured finding:* a safe **centroid-insertion** interior refinement preserves volume/validity/boundary but **cannot reduce boundary edges** (max edge pinned at the input-surface resolution while interior tets explode 30k→378k chasing an unreachable target), so a real size bound requires boundary (encroachment) refinement — the divergence-prone Ruppert-3-D problem, confirmed genuinely hard, not a quick win |
