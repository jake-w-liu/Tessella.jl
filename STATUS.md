...
| 4 | size fields + optimization | **partial** | tet quality report ✓; Laplacian + ODT smoothing ✓; verified 2-3/3-2 flip primitives ✓; `optimize_flips!` sliver reduction (309→169 flips alone, →63 with smoothing on a 300-pt cloud; volume/validity/min-dihedral-safe) ✓; weighted sliver exudation + domain-bounded 3-D size refinement WIP — **two concrete approaches built and measured to fail the size bound** (see Stage-4 measured finding below), so no size-refiner is shipped (a non-converging one would violate the correctness bar); the correct algorithm is boundary-conforming Delaunay refinement (Shewchuk terminator), divergence-prone near small input angles — genuinely research-grade, not a quick win |
...
Two concrete size-refinement approaches were **built and measured** on a convex box `[0,4]³` (target max-edge `hmax`); both provably fail to reach the bound, so neither is shipped (a non-converging refiner would fail the correctness bar):
...
**Conclusion (measured, not assumed):** a genuine 3-D size bound requires **boundary-conforming Delaunay refinement** — Steiner points inserted *and* boundary sub-faces split under an encroachment rule (Shewchuk's terminator), which is divergence-prone near small input angles and is a research-grade component, correctly out of single-session scope. *Test that would confirm a correct implementation:* on the box, `refine → maxedge ≤ hmax` for a sweep of `hmax`, with `validate.ok`, `is_delaunay3`, exact preserved volume, and bounded vertex count (termination).
