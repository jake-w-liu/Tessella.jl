...
**Suite green: 143,199 assertions** (`--check-bounds=yes`). Everything below is committed to
...
| 9 | *literal* ENC-COAX geometry | **main volumes DONE (2026-08-13)**; OCC sub-features remain a non-goal | the `.geo` is fully parametric (Box/Cylinder primitives) — Tessella now **parses the literal primitives directly and reconstructs+meshes the 3 main volumes (air/case/pin) natively at the exact fixture dimensions**, conforming + valid + all-filled, and the mesh loads in ASCENT (`validation/enclosure_literal/`, `LITERAL_HANDSHAKE_OK`, regression-pinned) — the geometry gmsh leaves empty. The tiny OCC-`BooleanFragments` sub-features (shield/bore cylinders, coupling slot, resistor rectangle, p1 disk + exact curved imprints) are the pure-Julia OpenCASCADE path PLAN §1/§6 lists as a non-goal; represented as tagged BC surfaces |
