...
- **Suite:** `julia --project=. -e 'using Pkg; Pkg.test()'` green — **143,026
  assertions** under `--check-bounds=yes` (verified this session; +28 `mesh_box`
  size-controlled mesher, +24 `mesh_box_regions` conforming multi-region / native
  box-CSG mesher, +22 `recover_boundary` general robust boundary recovery).
