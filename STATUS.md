...
**Suite green: 143,203 assertions** (`--check-bounds=yes`). Everything below is committed to
...
| 9 | *literal* ENC-COAX geometry | **main volumes DONE (2026-08-13)**; OCC sub-features remain a non-goal | the `.geo` is fully parametric (Box/Cylinder primitives) — Tessella now **parses the literal primitives directly and reconstructs+meshes 4 literal volumes (pin/slot/air/case) natively at the exact fixture dimensions** — including the thin coupling-**slot** sub-feature — conforming + valid + all-filled, and the mesh loads in ASCENT (`validation/enclosure_literal/`, `LITERAL_HANDSHAKE_OK`, regression-pinned) — the geometry gmsh leaves empty. The Box/Cylinder sub-features are natively reconstructible; only the exact OCC-`BooleanFragments` *imprint interfaces* + the surface features (resistor rectangle, p1 disk) need the pure-Julia OpenCASCADE path PLAN §1/§6 lists as a non-goal (represented as tagged BC surfaces) |
