...
| 3 | 3-D Delaunay + **robust boundary recovery** + slivers | **kernel + filling DONE; conforming partitions (planar + curved-convex interfaces) DONE; general recovery + slivers WIP** | 3-D empty-circumsphere oracle ✓; convex/non-convex/genus-1/thin/multi-region fills validated ✓; representative coax junction all volumes filled ✓; **`tetrahedralize_conforming`** — exact-coordinate Delaunay (no jitter) + per-region ray-cast tagging → interfaces are *shared* Delaunay faces (verified manifold + exact volume + every region filled on the **3-region pin/air/case enclosure topology**, incl. the pin's *curved N-gon* interface — all 4·nθ faces shared, no Steiner points) ✓, **enabled by the corrected SoS** (jitter of `perturb=true` moved interface vertices off-plane, destroying conformance); general constrained recovery for interfaces *not* expressible as Delaunay faces (non-convex regions, or a feature crossing a region boundary — the literal coax feed-through) OPEN |
...
- **2026-08-11** — **Audit + feature pass, exact-predicate SoS completed, and the
  ...
  - **The conforming-interface prediction above came true.** With the SoS predicates
    ...
    open. Full `Pkg.test()` green: **142,933 assertions** (+ conforming tests).
