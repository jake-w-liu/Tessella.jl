...
- **Suite:** `julia --project=. -e 'using Pkg; Pkg.test()'` green — **143,085
  assertions** under `--check-bounds=yes` (verified this session; +28 `mesh_box`,
  +24 `mesh_box_regions`, +22+12 `recover_boundary` (+ `steiner=true` fan fallback;
  + non-star-shaped prism breadth), +32 `mesh_boolean` native mesh-CSG, +11
  end-to-end native-pipeline integration).
