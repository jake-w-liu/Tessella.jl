...
│   ├── MeshQuadrature bounded Gmsh-shaped simplex reference integration rules
│   ├── MeshFunctionSpaces first-order Lagrange/H1/Whitney bases,
│   │                     orientations, and global node/edge keys
...
filters, classified node subsets, parametric coordinates, and nondefault Julia task
partitioning remain explicit blockers rather than fabricated metadata.
...
Reference function-space queries cover types 1, 2, and 4 with isoparametric or
explicit-order-one Lagrange functions and gradients, order-one hierarchical H1
functions and gradients, and lowest-order H(curl) functions and curls. Values use
Gmsh's orientation-major layout. Hierarchical orientation indices are lexicographic
ranks of primary node tags. Lagrange/H1 keys are dense node tags; H(curl) keys are
...
covers every fixed-node Point, Line, Triangle, and Tetrahedron type. Economical
`Gauss0` through `Gauss5` rules match Gmsh 4.15.2; bounded
`CompositeGaussN` rules use native Gauss--Legendre/Duffy construction with checked
point counts. Economical simplex rules above order five, non-simplex quadrature,
higher-order/non-simplex function spaces, entity-filtered orientation/key results,
and nondefault task partitioning remain pending.
...
fixed element type/property and simplex reference-quadrature lookup,
