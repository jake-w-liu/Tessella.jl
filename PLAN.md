...
{dx,dy,dz} { ListOfShapes ExtrudeParameters }` runs Gmsh's built-in kernel
...
surface tag burn and top-surface re-tag), and the returned list appends the
...
per-entity meshing parameters; rotational, twist, boundary-layer, pipe,
...
extension that pinned Gmsh lacks. Translational `Extrude` runs as both a
...
non-translational extrusions and fillets, allocator reads after
