...
| P3 | built-in/OCC-equivalent CAD, BREP/NURBS, imports, Booleans, transforms, `.geo` execution | IN PROGRESS — NURBS evaluation and STEP/IGES NURBS import (B_SPLINE / IGES 126/128) with IGES export, classified STEP/IGES box/sphere/cylinder/cone solids, expression-, numeric-list-, and tracked-tag-allocator-backed Point/Line/Circle/Ellipse/Loop/Plane Surface/Surface/Ruled Surface/Surface Loop/Volume with checked `SetMaxTag`, positive Point `MeshSize`, explicit-topology `PointsOf`, topology-derived Physical groups, global automatic Physical tags, owned operation-time Boolean operands with complete Delete cleanup, Box/Cylinder/Sphere/Cone/Boolean `.geo` solids, explicit-entity Translate/Dilate/Rotate/Symmetry with Gmsh's shape-list grammar/Duplicata/Coherence, translational `Extrude` for points/curves/planar surfaces with Gmsh's tag allocation, output lists, signed-generatrix and merge semantics plus `Layers`/`Recombine`/`ScaleLast`/`QuadTri*`/`Using` parameters, `Circle`/`Ellipse` arcs with Gmsh's `EndCurve` control-point/`Plane`-normal semantics and `Surface`/`Ruled Surface` filling including `In Sphere`/`Using Point` sphere-center metadata, and straight-curve or planar-surface periodic `.geo` execution, mesh Booleans/transforms; unrecognized CAD topology remains pending implementation |
...
instead of returning approximate fallback data. Other curved entities, implicit
